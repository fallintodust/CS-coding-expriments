...
void test_distribution(const char* dist_name, int* data, int size) {
    printf("\n--- %s测试 ---\n", dist_name);
    
    // 基本排序算法
    benchmark_sort(quick_sort, "快速排序", data, size);
    benchmark_sort(merge_sort, "归并排序", data, size);
    
    // 优化版本
    benchmark_sort(quick_sort_opt, "优化快速排序", data, size);
}
...
int main() {
    ...
    test_distribution("随机数据", data, ARRAY_SIZE);
    ...
    test_distribution("近乎有序数据", data, ARRAY_SIZE);
    
    // 3. 逆序数据测试
    for(int i = 0; i < ARRAY_SIZE; i++) {
        data[i] = ARRAY_SIZE - i;
    }
    test_distribution("逆序数据", data, ARRAY_SIZE);
